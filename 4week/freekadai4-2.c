...
int main() {
	...
	BYTE *averageData = (BYTE*)malloc(imageByte);
	int areaData[n * n];
	...
	for (x = 0; x < width; x++) {
		for (y = 0; y < height; y++) {
			...
			averageData[3 * (x + y * width)] = getAverage(areaData);			
		}
	}
	...
	for (x = 0; x < width; x++) {
		for (y = 0; y < height; y++) {
			...
			averageData[3 * (x + y * width) + 1] = getAverage(areaData);
		}
	}
	...
	for (x = 0; x < width; x++) {
		for (y = 0; y < height; y++) {
			...
			averageData[3 * (x + y * width) + 2] = getAverage(areaData);
		}
	}
	...
	if (writeJpeg("free4-2result.jpg", averageData, width, height, bpp, 100) == 1) {
		printf("success\n");
	} else {
		printf("not success\n");
	}
